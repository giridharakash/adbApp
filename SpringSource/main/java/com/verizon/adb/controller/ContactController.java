...
public class ContactController {
	...
	public ModelAndView handleAddContact(@ModelAttribute("contact") @Valid Contact contact,
		...
		if (contactService.existsByEmailId(contact.getEmailId())) {
			// validationResults.addError(new ObjectError("emailId","EmailId Already
			// Exists"));
			validationResults.rejectValue("emailId", "error.contact", "Mail ID Exists");
		}

		if (contactService.existsByMobileNumber(contact.getMobileNumber())) {
			// validationResults.addError(new ObjectError("mobileNumber","Mobile Number
			// Already Exists"));
			validationResults.rejectValue("mobileNumber", "error.contact", "Number Exists");
		}
